...
from model import complex_resnet18
import layers
import numpy as np
import pickle
...
class SAMPLEData(torch.utils.data.Dataset):
  def __init__(self,
    ...
    self.is_mstar = is_mstar
    self.filter_azimuth = filter_azimuth

    with open(os.path.join(root_dir, db_file), 'rb') as f:
      self.db = pickle.load(f)

    if isinstance(self.db, list) and 'class' not in self.db[0]:
      # find classes if there is none
      self.name2class = {}
      cls = 0
      for d in self.db:
    ...
    if isinstance(self.db, list):
      ...
      # this is m_star and for new db formats where we have a dict of train and val 
      assert(self.is_mstar)
      if self.filter_azimuth:
        self.db['train'] = [x for x in self.db['train'] if x['azimuth'] < np.pi/2]
        self.db['val'] = [x for x in self.db['val'] if x['azimuth'] < np.pi/2]
      train_len = len(self.db['train'])
      self.db = self.db['train'] + self.db['val']
      total_objects = len(self.db)
      indices = np.arange(total_objects)
    else:
      raise ValueError(f"Uknown type for self.db: {type(self.db)}")
    ...
    if self.is_mstar:
      # normalize
      global_max, global_min = np.finfo(float).min,np.finfo(float).max
      for index in train_indices:
        image = self.db[index]['image']
        mag = np.sqrt(image.real**2 + image.imag**2)
        im = 20*np.log10(mag + 1e-3)
        global_max, global_min = max(global_max, im.max()), min(global_min, im.min())
      self.global_max, self.global_min = global_max, global_min

      for i in range(total_objects):
        image = self.db[i]['image']
        mag = np.sqrt(image.real**2 + image.imag**2)
        im = 20*np.log10(mag+1e-3)
        im = (im - global_min) / (global_max - global_min)
        im = np.uint8(im * 255.)
        self.db[i]['image'] = np.stack([im, im, im], axis=2)
  ...
  def __getitem__(self, index):
    ...
    if self.is_regression:
      az = data['azimuth'] if self.is_mstar else data['azimuth'].item()
      return self.transform(image), az # azimuth in radian
    else:
      return self.transform(image), data['class']


def augment_magnitude(im, epsilon=0.1, noise_type='speckle',
                      transform='id', random_monotonic=True, Q=256, p=1.0):
  """
  Args:
...
class Augmentor:
  ...
  def quantize_and_augment(self, tensor, max_to_use=None, min_to_use=None):
    ...
    aug_tensor = augment_magnitude(tensor, self.epsilon, self.noise_type, 'id', self.random_monotonic, Q=self.Q, p=self.p)
  ...
  def __call__(self, im):
    if self.is_complex:
      ...
      return Image.fromarray(np.uint8(augment_magnitude(im, self.epsilon, self.noise_type, 'id', self.random_monotonic, Q=self.Q, p=self.p)))
...
class SimCLRModel(pl.LightningModule):
  ...
  def configure_optimizers(self):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, self.args.max_epochs // self.args.cosinelr_div, eta_min=self.args.lr_eta_min
    )
...
def main(args):
      ...
      view_transform = torchvision.transforms.Compose(
          [
            ...
            SimCLRViewTransform(
              ...
              normalize=None,
              min_scale=1.0 if args.simclr_jitter_only else 0.08,
              hf_prob=0.0 if args.simclr_jitter_only else 0.5,
            )
          ]
      )
    else:
  ...
  train_dataset_simclr = LightlyDataset.from_torch_dataset(
      SAMPLEData(db_file=args.db_file, is_regression=args.is_regression,
                 syn2real=args.syn2real, is_mstar=args.is_mstar,
                 filter_azimuth=args.filter_azimuth),
      transform=transform
  )

  train_classifier_transform = view_transform if args.augment_classifier else val_transform
  ...
  train_dataset_classifier = SAMPLEData(
      ...
      is_mstar=args.is_mstar, filter_azimuth=args.filter_azimuth,
  )
...
if __name__ == '__main__':
  ...
  parser.add_argument("--is_mstar", action='store_true', default=False,
                      help="if it is the full mstar dataset it should be true")
  parser.add_argument("--use_simclr_transform", action='store_true', default=False)
  parser.add_argument("--simclr_jitter_only", action='store_true', default=False)
  parser.add_argument("--augment_prob", type=float, default=1.0)
  parser.add_argument("--lr_eta_min", type=float, default=0)
  parser.add_argument("--cosinelr_div", type=int, default=1)
  parser.add_argument("--filter_azimuth", action='store_true', default=False)
