...
class ComplexAdaptiveAvgPool2d(nn.Module):
  ...
  def __repr__(self):
    return 'ComplexAdaptiveAveragePooling2d'

  def forward(self, x):
    x_real = self.avg_pool(x[:,0])
    x_imag = self.avg_pool(x[:,1])
    return th.stack([x_real, x_imag], dim=1)

class ComplexConv(nn.Module):
    # Our complex convolution implementation
    def __init__(self, in_channels, num_filters, kernel_size, stride=(1, 1), padding=0, dilation=1, groups=1, reflect=False, bias=False, new_init=False, use_groups_init=False, fan_in=False, *args, **kwargs):
        ...
        self.A = nn.Conv2d(in_channels, num_filters, kernel_size,
                           stride=stride, padding=padding, groups=groups, bias=bias)
        self.B = nn.Conv2d(in_channels, num_filters, kernel_size,
                           stride=stride, padding=padding, groups=groups, bias=bias)
        ...
        if new_init:
            ...
            gain = 1/np.sqrt(2)
            with th.no_grad():
                std = gain / np.sqrt(kernel_size * kernel_size * c)
...
class DivLayer(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size, stride=(1, 1), padding=0, dilation=1, groups=1, reflect=False, use_one_filter=True, new_init=False):
        ...
        if self.use_one_filter:
            self.conv = ComplexConv(
                in_channels, 1, kernel_size, stride, padding, dilation, groups, reflect=reflect, new_init=new_init)
        else:
            self.conv = ComplexConv(in_channels, in_channels, kernel_size,
...
class ConjugateLayer(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size, stride=(1, 1), padding=0, dilation=1, groups=1, reflect=False, use_one_filter=False, new_init=False):
        ...
        if self.use_one_filter:
            ...
            self.conv = conv(in_channels, in_channels, kernel_size,
                             stride, padding, dilation, groups, reflect=reflect, new_init=new_init)
