  ...
  for gen_seed in args.gen_seeds:
    ...
    for i in tqdm(range(args.num_samples)):
      for j in range(args.num_neighbours):
        ...
        if args.crop:
          sample_image = center_crop(sample_image, args.crop)
          target_image = center_crop(target_image, args.crop)
        else:
          target_image = skimage.transform.resize(target_image, sample_image.shape)
        cu_value = 0.25
        sample_image = sample_image * 255
        target_image *= 255
        sample_image = findpeaks.stats.kuan_filter(sample_image, win_size=7, cu=cu_value)
        target_image = findpeaks.stats.kuan_filter(target_image, win_size=7, cu=cu_value)
        ssim = skimage.metrics.structural_similarity(
            sample_image, target_image, data_range=255.0, win_size=7
        )
        ssims.append(ssim)
